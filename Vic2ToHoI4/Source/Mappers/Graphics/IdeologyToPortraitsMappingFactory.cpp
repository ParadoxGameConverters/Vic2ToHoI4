...
Mappers::IdeologyToPortraitsMappingFactory::IdeologyToPortraitsMappingFactory()
{
	registerRegex(commonItems::catchallRegex, [this](const std::string& ideology, std::istream& theStream) {
		theMap.insert(make_pair(ideology, commonItems::stringList(theStream).getStrings()));
	});
}


std::map<std::string, std::vector<std::string>> Mappers::IdeologyToPortraitsMappingFactory::importMapping(std::istream& theStream)
{
	theMap.clear();
