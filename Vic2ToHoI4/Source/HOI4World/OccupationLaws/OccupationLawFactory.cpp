...
HoI4::OccupationLaw::Factory::Factory()
{
	...
	registerKeyword("sound_effect", [this](const std::string& unused, std::istream& theStream) {
	...
	registerKeyword("state_modifier", [this](const std::string& unused, std::istream& theStream) {
		occupationLaw->setStateModifier(commonItems::stringOfItem(theStream).getString());
	});
	registerKeyword("suppressed_state_modifier", [this](const std::string& unused, std::istream& theStream) {
