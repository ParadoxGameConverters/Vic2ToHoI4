...
void HoI4::Events::addMinisterRevolutionEvents(const std::set<std::string>& majorIdeologies)
{
	...
	{
		addDemocraticMinisterRevolutionEvents();
	}
}


void HoI4::Events::addDemocraticMinisterRevolutionEvents()
{
	Event opposition;
...
void HoI4::Events::addSuppressedEvents(const std::set<std::string>& majorIdeologies)
{
	...
	if (numRelevantIdeologies > 0)
	{
		...
		for (const auto& ideology: majorIdeologies)
		{
			...
			HoI4Localisation::copyEventLocalisations("abandon_neutral.d_" + ideology,
...
void HoI4::Events::updateGenericEventOne(Event& eventOne) const
{
...
void HoI4::Events::updateGenericEventTwo(Event& eventTwo, const std::set<std::string>& majorIdeologies) const
{
...
void HoI4::Events::updateGenericEventFive(Event& eventFive, const std::set<std::string>& majorIdeologies) const
{
	...
	if (majorIdeologies.count("democratic"))
	{
		...
		option.giveAiChance(
			...
			"\t\t\tbase = 100\n"
			"\t\t}");
		option.giveScriptBlock("FROM = {\n"
			"\t\t\tadd_to_faction = ROOT\n"
			"\t\t\tcountry_event = { id = generic.6 hours = 6 }\n"
			"\t\t}"
		);
		option.giveScriptBlock("add_ai_strategy = {\n"
			"\t\t\ttype = alliance\n"
			"\t\t\tid = FROM\n"
			"\t\t\tvalue = 200\n"
			"\t\t}\n"
	...
	std::string trigger = "is_in_faction = no\n"
	"\t\t\tNOT = {\n";
	for (const auto& ideology : majorIdeologies)
	{
		if (ideology == "neutrality")
	...
	bothDifferent.giveAiChance(
		...
		"\t\t\t}\n"
		"\t\t}");
	bothDifferent.giveScriptBlock("FROM = {\n"
		"\t\t\tadd_to_faction = ROOT\n"
		"\t\t\tcountry_event = { id = generic.6 hours = 6 } }\n"
	"\t\t}"
	);
